class QuizGame6 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.html("question6", "./src/components/questions/question6.html");
        this.load.html("contador", "./src/components/contador.html");
        this.load.html("answer6", "./src/components/questions/answer6.html");
    ...
    create() {
        ...
        this.questao6 = this.add.dom((this.game.config.width) / 2, (this.game.config.height) / 2).createFromCache('question6').setScale(window.innerWidth * 0.0025);
        this.feedback6 = this.add.dom((this.game.config.width - (this.game.config.width + 50)) / 2, (this.game.config.height - this.game.config.height) / 2).createFromCache('answer6');
        ...
        this.intervalo = setInterval(() => {
            if (document.getElementById('contador').innerHTML == 0 && document.getElementById('textAnswer').innerHTML == "") {
                ...
                document.getElementById("answer6").classList.remove("d-none")
        ...
        document.getElementById("answer6a").addEventListener("click", (event) => {
        ...
        document.getElementById("answer6b").addEventListener("click", (event) => {
            this.resposta = "Quase lá! Tente novamente."; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somerro.play(); //tocar som de erro
        });
        document.getElementById("answer6c").addEventListener("click", (event) => {
            this.resposta = "Quase lá! Tente novamente."; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somerro.play(); //tocar som de erro
        });
        document.getElementById("answer6d").addEventListener("click", (event) => {
            this.resposta = "Quase lá! Tente novamente."; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somerro.play(); //tocar som de erro
        });
        ...
        play.addEventListener("click", (event) => {
            ...
            this.scene.start('Churrasqueira'); //troca para nova cena (quiz ou cenário)
        });
        ...
        document.getElementById("openConfirmar").addEventListener("click", (event) => {
            ...
            clearInterval(this.intervalo);
            this.scene.stop("QuizGame6");
        });

        //Botão para voltar ao menu na tela de feedback, assim que clicado ele abre o popup de confirmação
        var voltarMenu = document.getElementById("voltarMenu");
        voltarMenu.addEventListener("click", (event) => {
            this.voltarMenu();
        });
    ...
    armazenaTrofeu(trofeuValor) {
        ...
        this.trofeusArray.splice(5, 1, trofeuValor);
