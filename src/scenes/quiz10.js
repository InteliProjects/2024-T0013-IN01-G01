class QuizGame10 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.html("question10", "./src/components/questions/question10.html");
        this.load.html("contador", "./src/components/contador.html");
        this.load.html("answer10", "./src/components/questions/answer10.html");
    ...
    create() {
        ...
        this.questao10 = this.add.dom((this.game.config.width) / 2, (this.game.config.height) / 2).createFromCache('question10').setScale(window.innerWidth * 0.0025);
        this.feedback10 = this.add.dom((this.game.config.width - (this.game.config.width + 50)) / 2, (this.game.config.height - this.game.config.height) / 2).createFromCache('answer10');
        ...
        this.intervalo = setInterval(() => {
            if (document.getElementById('contador').innerHTML == 0 && document.getElementById('textAnswer').innerHTML == "") {
                ...
                document.getElementById("answer10").classList.remove("d-none")
        ...
        document.getElementById("answer10a").addEventListener("click", (event) => {
        ...
        document.getElementById("answer10b").addEventListener("click", (event) => {
            this.resposta = "Resposta Correta!"; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somacerto.play(); //tocar som de acerto
        });
        document.getElementById("answer10c").addEventListener("click", (event) => {
            this.resposta = "Quase lá! Tente novamente."; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somerro.play(); //tocar som de erro
        });
        document.getElementById("answer10d").addEventListener("click", (event) => {
            this.resposta = "Quase lá! Tente novamente."; //texto da resposta
            this.tempoResposta = document.getElementById('contador').innerHTML; //Pegar o tempo atual que a pergunta foi respondida
            this.somerro.play(); //tocar som de erro
        });

        //Lógica para passar para a tela final
        var play = document.getElementById("nextLevel");
        play.addEventListener("click", (event) => {
            clearInterval(this.intervalo);
            this.trofeuHud.destroy();
            this.scene.start('TelaFinal'); //troca para nova cena (quiz ou cenário)
        });
    ...
    armazenaTrofeu(trofeuValor) {
        ...
        this.trofeusArray.splice(9, 1, trofeuValor);
