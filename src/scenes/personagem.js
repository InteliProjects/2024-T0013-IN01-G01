class Personagem extends Phaser.Scene {
    ...
    create() {
        ...
        // Adição de texto
        this.textoEscolha = this.add.text((innerWidth / 2), 100, "Escolha seu personagem!", { fill: '#000' }).setOrigin(0.5).setFont('25px "Montserrat"');
        ...
        this.lavinia.setInteractive();
        this.alex.setInteractive();
        this.enzo.setInteractive();
        this.mauro.setInteractive();
        this.bruna.setInteractive();

        // Configurar o que lavinia deve fazer ao ser clicada
        this.lavinia.on("pointerup", () => this.personagemEscolhido('lavinia'));

        // Configurar o que alex deve fazer ao ser clicado
        this.alex.on("pointerup", () => this.personagemEscolhido('alex'));

        // Configurar o que enzo deve fazer ao ser clicado
        this.enzo.on("pointerup", () => this.personagemEscolhido('enzo'));

        this.mauro.on("pointerup", () => this.personagemEscolhido('mauro'));
        this.bruna.on("pointerup", () => this.personagemEscolhido('bruna'));
    ...
    personagemEscolhido(personagem) {
        ...
        switch (personagem) {
            case 'lavinia':
                personagemEscolhido = 'lavinia';
                break;
            case 'alex':
                personagemEscolhido = 'alex';
                break;
            case 'enzo':
                personagemEscolhido = 'enzo';
                break;
            case 'mauro':
                personagemEscolhido = 'mauro';
                break;
            case 'bruna':
                personagemEscolhido = 'bruna';
                break;
            default:
                personagemEscolhido = 'alex';
                break;
        ...
        if (personagemEscolhido) {
            localStorage.setItem("Personagem", personagemEscolhido);
        }

        // Iniciar a próxima cena
        this.comecarProximaCena("Tutorial");
    }

    // Começa a próxima cena
    comecarProximaCena(cena) {
        this.scene.switch(cena);
    }
